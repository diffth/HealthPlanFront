...
// 
    const callNboardListApi = (page) => {
        axios.get(`http://localhost:8080/api/slist?page=${page}`)
            .then(response => {
                try {
    ...
    const callSboardListApi = (page) => {
        axios.get(`http://localhost:8080/api/slist?page=${page}&searchType=${searchtype}&keyword=${keyword}`)
            .then(response => {
                try {
                    setAppend_NboardList(nBoardListAppend(response.data));
                    setTotalPages(response.data.pageMaker.totalCount);
